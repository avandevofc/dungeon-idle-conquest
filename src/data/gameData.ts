...
export const ENEMY_NAMES: Record<DungeonTheme, { mobs: string[]; boss: string }> = {
  ...
  'Cripta': {
    ...
    boss: 'Dragão Esquelético Necromante',
  },
  'Infernal': {
...
export const MANA_UPGRADES: ManaUpgradeDef[] = [
  ...
  },
  {
    ...
    name: 'Olho Crítico',
    icon: '👁️',
    description: '+3% Chance de Crítico',
    baseCost: 8,
    costScale: 1.6,
    effect: (level) => level * 3,
    effectLabel: (level) => `+${level * 3}% Crítico`,
  },
  {
    id: 'swift_blades',
    name: 'Lâminas Velozes',
    icon: '⚡',
    description: '+8% Velocidade de Ataque',
    baseCost: 10,
    costScale: 1.7,
    effect: (level) => level * 8,
    effectLabel: (level) => `+${level * 8}% Vel. Ataque`,
  },
  {
    id: 'mana_surge',
    name: 'Surto de Mana',
    icon: '💎',
    description: '+15% Drop de Mana em Bosses',
    baseCost: 12,
    costScale: 1.8,
    effect: (level) => 1 + level * 0.15,
    effectLabel: (level) => `+${level * 15}% Mana Boss`,
  },
  {
    id: 'iron_skin',
    name: 'Pele de Ferro',
    icon: '🛡️',
    description: '+20% HP dos Heróis',
    baseCost: 8,
    costScale: 1.6,
    effect: (level) => 1 + level * 0.20,
    effectLabel: (level) => `+${level * 20}% HP`,
  },
  {
    id: 'treasure_sense',
    name: 'Sentido do Tesouro',
    icon: '📦',
    description: '+5% Chance de Item',
    baseCost: 15,
    costScale: 2.0,
    effect: (level) => level * 5,
    effectLabel: (level) => `+${level * 5}% Item Drop`,
  },
  {
    id: 'critical_power',
    name: 'Poder Crítico',
    ...
    effectLabel: (level) => `+${level * 25}% Dano Crítico`,
  },
];
...
export const BOSS_SPRITES: Record<DungeonTheme, string> = {
  ...
  'Abismo': '🐙',
  'Celestial': '👼',
  'Cripta': '🐉', // Dragão Esquelético Necromante
  'Infernal': '😈',
  'Dimensional': '🌀',
};
