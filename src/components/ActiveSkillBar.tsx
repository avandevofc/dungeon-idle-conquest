...
import { ActiveSkillState, PetState } from '../types';
import { HERO_DEFS } from '../data/gameData';
import { PET_DEFS, PET_ELEMENTS, PET_LEVEL_COST } from '../data/pets';
...
function PetSkillButton({ pet, petState, onActivate }: { key?: React.Key; pet: typeof PET_DEFS[0]; petState: PetState; onActivate: () => void }) {
  const isOnCooldown = (petState as any).skillCooldownRemaining > 0;
  const cooldownPercent = isOnCooldown ? ((petState as any).skillCooldownRemaining / pet.activeSkill.cooldown) * 100 : 0;
  const elementInfo = PET_ELEMENTS[pet.element];
  ...
  return (
    <div className="relative group">
      <button
        ...
        className="w-12 h-12 flex items-center justify-center transition-all relative overflow-hidden"
        style={{
          background: isOnCooldown ? 'rgba(30,30,50,0.5)' : `${elementInfo.color}20`,
          border: `2px solid ${isOnCooldown ? '#333355' : elementInfo.color}`,
          boxShadow: isOnCooldown ? 'none' : `0 0 12px ${elementInfo.color}40`,
          cursor: isOnCooldown ? 'not-allowed' : 'pointer',
        }}
        title={`${pet.activeSkill.name}: ${pet.activeSkill.description}`}
      >
        {isOnCooldown && (
          <div
            className="absolute inset-0 transition-all duration-1000"
        ...
        <span className="relative z-10 text-base">{pet.activeSkill.icon}</span>
      </button>
      {isOnCooldown && (
        <div className="absolute -top-1 -right-1 px-1 py-0.5 text-[8px] font-bold" style={{ background: 'rgba(0,0,0,0.8)', color: '#94a3b8' }}>
          {(petState as any).skillCooldownRemaining}s
        </div>
      )}
      <div className="absolute -top-2 left-1/2 -translate-x-1/2 -translate-y-full px-3 py-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 w-48" style={{ background: 'rgba(10,10,18,0.95)', border: `2px solid ${elementInfo.color}44` }}>
        <div className="text-[11px] font-bold" style={{ color: elementInfo.color }}>{pet.activeSkill.name}</div>
        <div className="text-[10px] text-[#94a3b8] mt-1">{pet.activeSkill.description}</div>
        <div className="flex items-center gap-2 mt-1.5 text-[9px] text-[#475569]">
          <span>🐾 Pet Skill</span>
          <span>⏱️ {pet.activeSkill.cooldown}s</span>
        </div>
      </div>
    </div>
  );
...
export function ActiveSkillBar({ heroes, activeSkills, onActivateSkill, activePet, pets, onActivatePetSkill }: Props) {
  ...
  // Get active pet info
  const activePetDef = activePet ? PET_DEFS.find(d => d.id === activePet) : null;
  const activePetState = activePet ? pets.find(p => p.id === activePet) : null;

  if (availableSkills.length === 0 && !activePetDef) return null;
  ...
  return (
    <div className="glass-card-static p-3">
      ...
      <div className="flex gap-2 flex-wrap">
        {/* Pet skill first */}
        {activePetDef && activePetState && (
          <PetSkillButton
            key="pet_skill"
            pet={activePetDef}
            petState={activePetState}
            onActivate={onActivatePetSkill}
          />
        )}
        {/* Hero skills */}
